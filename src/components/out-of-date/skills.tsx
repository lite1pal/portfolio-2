export default function Skills() {
  return (
    <div className="mx-auto flex w-fit flex-col items-center justify-center gap-5 lg:flex-row">
      <SkillCard
        svgIcon={
          <svg
            className="h-12 w-12 text-primary dark:text-primary-dark"
            aria-hidden="true"
            xmlns="http://www.w3.org/2000/svg"
            fill="currentColor"
            viewBox="0 0 20 20"
          >
            <path d="M10 0C4.612 0 0 5.336 0 7c0 1.742 3.546 7 10 7 6.454 0 10-5.258 10-7 0-1.664-4.612-7-10-7Zm0 10a3 3 0 1 1 0-6 3 3 0 0 1 0 6Z" />
          </svg>
        }
        h={"Keen eye on Design"}
        p={
          "Able to come with innovative solutions for your design to attract more clients"
        }
      />
      <SkillCard
        svgIcon={
          <svg
            className="h-12 w-12 text-primary dark:text-primary-dark"
            aria-hidden="true"
            xmlns="http://www.w3.org/2000/svg"
            fill="none"
            viewBox="0 0 22 21"
          >
            <path
              stroke="currentColor"
              strokeLinecap="round"
              strokeWidth="2"
              d="M7.24 7.194a24.16 24.16 0 0 1 3.72-3.062m0 0c3.443-2.277 6.732-2.969 8.24-1.46 2.054 2.053.03 7.407-4.522 11.959-4.552 4.551-9.906 6.576-11.96 4.522C1.223 17.658 1.89 14.412 4.121 11m6.838-6.868c-3.443-2.277-6.732-2.969-8.24-1.46-2.054 2.053-.03 7.407 4.522 11.959m3.718-10.499a24.16 24.16 0 0 1 3.719 3.062M17.798 11c2.23 3.412 2.898 6.658 1.402 8.153-1.502 1.503-4.771.822-8.2-1.433m1-6.808a1 1 0 1 1-2 0 1 1 0 0 1 2 0Z"
            />
          </svg>
        }
        h={"Extensive Skillset"}
        p={
          "Whether you want to create a landing page or an entire front-end, I got you covered"
        }
      />
      <SkillCard
        svgIcon={
          <svg
            className="h-12 w-12 text-primary dark:text-primary-dark"
            aria-hidden="true"
            xmlns="http://www.w3.org/2000/svg"
            fill="currentColor"
            viewBox="0 0 20 20"
          >
            <path d="M18 7.5h-.423l-.452-1.09.3-.3a1.5 1.5 0 0 0 0-2.121L16.01 2.575a1.5 1.5 0 0 0-2.121 0l-.3.3-1.089-.452V2A1.5 1.5 0 0 0 11 .5H9A1.5 1.5 0 0 0 7.5 2v.423l-1.09.452-.3-.3a1.5 1.5 0 0 0-2.121 0L2.576 3.99a1.5 1.5 0 0 0 0 2.121l.3.3L2.423 7.5H2A1.5 1.5 0 0 0 .5 9v2A1.5 1.5 0 0 0 2 12.5h.423l.452 1.09-.3.3a1.5 1.5 0 0 0 0 2.121l1.415 1.413a1.5 1.5 0 0 0 2.121 0l.3-.3 1.09.452V18A1.5 1.5 0 0 0 9 19.5h2a1.5 1.5 0 0 0 1.5-1.5v-.423l1.09-.452.3.3a1.5 1.5 0 0 0 2.121 0l1.415-1.414a1.5 1.5 0 0 0 0-2.121l-.3-.3.452-1.09H18a1.5 1.5 0 0 0 1.5-1.5V9A1.5 1.5 0 0 0 18 7.5Zm-8 6a3.5 3.5 0 1 1 0-7 3.5 3.5 0 0 1 0 7Z" />
          </svg>
        }
        h={"Strong Dedication"}
        p={
          "Rest assured that everything will be delived in the highest-quality possible"
        }
      />
    </div>
  );
}

function SkillCard({ svgIcon, h, p }: { svgIcon: any; h: string; p: string }) {
  return (
    <div className="group flex h-64 w-72 cursor-default flex-col gap-6 rounded-lg bg-slate-200 p-10 transition  duration-300 hover:bg-slate-100 dark:bg-slate-800 dark:hover:bg-slate-900">
      {svgIcon}
      <div className="text-lg text-slate-800 dark:text-slate-50 dark:group-hover:text-slate-50">
        {h}
      </div>
      <div className="text-sm text-slate-500 dark:text-slate-400 dark:group-hover:text-slate-300">
        {p}
      </div>
    </div>
  );
}
